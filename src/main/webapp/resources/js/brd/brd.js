...
brd = (()=>{
	...
	let setContentView=()=>{
			$('head').html(brd_vue.brd_head({ctx: '/web',css: $.css(), img: $.img()}))
...
//		        '<input class="form-control mr-sm-2" type="text" placeholder="검색어를 입력해 주세요." aria-label="Search">'+
		...
		$('#pageing_form option[value="'+pxy.pageSize+'"]').attr('selected',true)
		$('#pageing_form').change(()=>{
		...
		if(pxy.existPrev){
        	...
        	for(i= pxy.startPage; i<=pxy.endpage; i++) {
        		if(pxy.pageNum==i){
    				...
    				.appendTo('#pagination')
    				.click(function(){
    					alert('현재 클릭 페이지 보기'+$(this).children('.page-link').text())
